...
class StoreKelasLabRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('create', KelasLab::class);
    }

    /**
     * Dosen hanya boleh membuka kelas untuk dirinya sendiri → paksa dosen_id miliknya.
     * Supervisor wajib mengirim dosen_id valid (atas permintaan Dosen ybs.).
     */
    protected function prepareForValidation(): void
    {
        if ($this->user()->role === 'dosen') {
            $this->merge(['dosen_id' => $this->user()->dosen?->id]);
        }
    }
    ...
    /**
     * Validasi bisnis (UC-02a): status ruangan & bentrok terhadap kelas lain + peminjaman disetujui.
     */
